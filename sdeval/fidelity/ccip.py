...
class CCIPMetrics:
    ...
    def diff_score(self, images: ImagesTyping) -> Tuple[float, float]:
        image_list: List[Image.Image] = load_images(images)
        if not image_list:
    ...
    def diff(self, images: ImagesTyping) -> float:
        diff, score = self.diff_score(images)
        return diff

    def score(self, images: ImagesTyping) -> float:
        diff, score = self.diff_score(images)
        return score
